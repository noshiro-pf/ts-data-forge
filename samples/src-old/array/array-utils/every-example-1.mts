...
assert.strictEqual(areAllStrings, true);
assert.deepStrictEqual(data, ['a', 'b', 3]);
assert.deepStrictEqual(empty, []);
assert.strictEqual(joined, undefined);
assert.deepStrictEqual(mixed, ['hello', 'world']);
assert.deepStrictEqual(numbers, [2, 4, 6, 8]);
assert.strictEqual(result2, true);
assert.deepStrictEqual(uppercase, ['HELLO', 'WORLD']);
