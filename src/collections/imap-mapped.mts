...
/**
 * Interface for an immutable map with custom key mapping and O(1) lookup
 * performance.
 *
 ...
 * native Map. This enables high-performance operations on maps with complex
 * keys while maintaining type safety and immutability.
 *
 * **Key Features:**
 *
 * - **Custom Key Types**: Use any type as keys by providing `toKey`/`fromKey`
 *   functions
 * - **O(1) Performance**: Maintains O(1) average-case performance for core
 *   operations
