...
/**
 * Interface for an immutable set with custom element mapping and O(1)
 * membership testing.
 *
 ...
 * JavaScript's native Set. This enables high-performance set operations on
 * complex elements while maintaining type safety and immutability.
 *
 * **Key Features:**
 *
 * - **Custom Element Types**: Use any type as set elements by providing
 *   `toKey`/`fromKey` functions
 * - **O(1) Performance**: Maintains O(1) average-case performance for membership
 ...
 * - **High Performance**: O(1) operations through efficient element
 *   transformation
