...
export const NonPositiveInt = {
  ...
  /**
   * Multiplies two non-positive integers.
   *
   * The result is clamped to the non-positive integer range.
   *
   * @example
   *
   * ```ts
   * assert.strictEqual(NonPositiveInt.mul(-5, -3), -15);
   *
   * assert.strictEqual(NonPositiveInt.mul(-5, 0), 0);
   * ```
   *
   * @param a - The first non-positive integer
   * @param b - The second non-positive integer
  ...
  /**
   * Divides one non-positive integer by another using floor division.
   *
   * The result is clamped to the non-positive integer range.
   *
   * @example
   *
   * ```ts
   * assert.strictEqual(NonPositiveInt.div(-10, -2), -5);
   *
   * assert.strictEqual(NonPositiveInt.div(-10, 2), -5);
   * ```
   *
   * @param a - The dividend non-positive integer
   * @param b - The divisor non-positive integer
   * @returns `⌊a / b⌋` clamped to the non-positive integer range
   */
  div,
} as const;
