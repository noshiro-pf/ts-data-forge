...
describe('NonPositiveSafeInt test', () => {
  describe(asNonPositiveSafeInt, () => {
    ...
    test('rejects values outside safe integer range', () => {
      ...
      expect(() =>
        asNonPositiveSafeInt(Number.MAX_SAFE_INTEGER + 1),
      ).toThrow(TypeError);
    });
    ...
    test('throws TypeError for invalid values', () => {
      ...
      expect(() =>
        asNonPositiveSafeInt(Number.MIN_SAFE_INTEGER - 1),
      ).toThrow(TypeError);
  ...
  describe("NonPositiveSafeInt.mul", () => {
    test('multiplies two non-positive integers', () => {
      expect(NonPositiveSafeInt.mul(-5, -3)).toBe(-15);

      expect(NonPositiveSafeInt.mul(-5, 0)).toBe(0);

      expect(NonPositiveSafeInt.mul(0, -5)).toBe(0);
    });
  });

  describe("NonPositiveSafeInt.div", () => {
    test('divides two non-positive integers', () => {
      expect(NonPositiveSafeInt.div(-10, -2)).toBe(-5);

      expect(NonPositiveSafeInt.div(-10, 2)).toBe(-5);

      expect(NonPositiveSafeInt.div(0, -5)).toBe(0);
    });
  });
